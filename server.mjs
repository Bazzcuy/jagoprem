...
const server = createServer(async (request, response) => {
  ...
  if (pathname === '/api/orders' && request.method === 'POST') {
    try {
      ...
      const customer = { name: user.name, email: user.email, whatsapp: String(body.customer?.whatsapp || '').trim().replace(/[\s-]/g, '').slice(0, 20), note: String(body.customer?.note || '').trim().slice(0, 500) }; if (!/^\+?[0-9]{9,15}$/.test(customer.whatsapp)) return sendJson(response, 400, { error: 'Nomor WhatsApp tidak valid. Isi hanya angka, panjang 9–15 digit, boleh diawali +.' });
