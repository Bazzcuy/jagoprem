...
function enhancedAccountTable() {
  const users = adminState.users || [];
  if (!activeAccountId && users.length) activeAccountId = users[0].id;
  const selected = users.find((user) => user.id === activeAccountId);
  return `<div class="account-workspace"><div class="panel account-list"><div class="panel-head"><div><h2>Akun pembeli</h2><span class="panel-meta">${users.length} akun terdaftar</span></div><input id="accountSearch" type="search" placeholder="Cari nama atau email..."></div><div class="table-wrap"><table><thead><tr><th>PEMBELI</th><th>BERGABUNG</th><th>PESANAN</th><th>TOTAL</th><th>STATUS</th><th></th></tr></thead><tbody>${users.map((user) => `<tr data-account-row="${user.id}"><td><b>${escapeHtml(user.name)}</b><br><small>${escapeHtml(user.email)}</small></td><td>${formatDateTime(user.createdAt)}</td><td>${user.orderCount}</td><td>${money(user.totalSpent)}</td><td><span class="account-status ${user.blocked ? 'blocked' : 'active'}">${user.blocked ? 'Diblokir' : 'Aktif'}</span></td><td><button class="account-detail-button" data-account-detail="${user.id}">Detail</button></td></tr>`).join('')}</tbody></table></div></div>${selected ? enhancedAccountDetail(selected) : ''}</div>`;
}

function enhancedAccountDetail(user) {
...
function enhancedChatWorkspace() {
...
function orderTable() {
  if (!adminState.orders.length) return '<div class="panel"><div class="panel-head"><h2>Pesanan</h2></div><div class="empty-admin">Belum ada pesanan.</div></div>';
  return `<div class="panel"><div class="panel-head"><h2>Pesanan</h2><span class="panel-meta">${adminState.orders.length} transaksi</span></div><div class="table-wrap"><table><thead><tr><th>ID</th><th>PELANGGAN</th><th>ITEM</th><th>TOTAL</th><th>STATUS</th></tr></thead><tbody>${adminState.orders.map((order) => { const statuses = order.status === 'expired' ? ['expired', 'pending', 'cancelled'] : ['pending', 'paid', 'completed', 'cancelled']; return `<tr><td><b>${escapeHtml(order.id)}</b><br><small>${formatDateTime(order.createdAt)}</small>${order.expiresAt && order.status === 'pending' ? `<br><small>Expired ${new Date(order.expiresAt).toLocaleTimeString('id-ID')}</small>` : ''}</td><td><b>${escapeHtml(order.customer?.name || '-')}</b><br>${escapeHtml(order.customer?.whatsapp || '-')}</td><td>${order.items.map((line) => `${escapeHtml(line.title || adminState.products.find((product) => product.id === line.id)?.title || line.id)}${line.ownGmail ? ' - Gmail sendiri' : ''}${line.reseller ? ' - Reseller' : ''} x${line.quantity}`).join('<br>')}</td><td>${money(order.total)}</td><td><select class="order-select status ${escapeHtml(order.status)}" data-order-status="${escapeHtml(order.id)}" data-previous-status="${escapeHtml(order.status)}">${statuses.map((status) => `<option value="${status}" ${order.status === status ? 'selected' : ''}>${status}</option>`).join('')}</select></td></tr>`; }).join('')}</tbody></table></div></div>`;
}

function accountDetail(user) {
  return `<aside class="panel account-detail"><div class="account-detail-head"><div><small>DETAIL AKUN</small><h2>${escapeHtml(user.name)}</h2><span>${escapeHtml(user.email)}</span></div><span class="account-status ${user.blocked ? 'blocked' : 'active'}">${user.blocked ? 'Diblokir' : 'Aktif'}</span></div><dl><div><dt>Terdaftar</dt><dd>${formatDateTime(user.createdAt)}</dd></div><div><dt>ID akun</dt><dd>${escapeHtml(user.id)}</dd></div><div><dt>ID perangkat</dt><dd>${escapeHtml(user.deviceId || '-')}</dd></div><div><dt>Total belanja</dt><dd>${money(user.totalSpent)}</dd></div></dl><button class="block-account ${user.blocked ? 'unblock' : ''}" data-block-account="${user.id}" data-blocked="${user.blocked}"><i data-lucide="${user.blocked ? 'shield-check' : 'ban'}"></i>${user.blocked ? 'Buka blokir akun' : 'Blokir akun'}</button><div class="account-orders"><h3>Riwayat pembelian</h3>${user.orders?.length ? user.orders.map((order) => `<article><div><b>${escapeHtml(order.id)}</b><span>${formatDateTime(order.createdAt)}</span></div><em class="status ${order.status}">${order.status}</em><p>${order.items.map((line) => `${escapeHtml(line.title || adminState.products.find((item) => item.id === line.id)?.title || line.id)} x${line.quantity}${line.reseller ? ' - reseller' : ''}`).join('<br>')}</p><strong>${money(order.total)}</strong></article>`).join('') : '<p class="empty-account-orders">Belum pernah membeli.</p>'}</div></aside>`;
}
...
function render(options = {}) {
  ...
  content.innerHTML = activeTab === 'overview' ? stats() + enhancedOrderTable() : activeTab === 'products' ? productTable() : activeTab === 'orders' ? enhancedOrderTable() : activeTab === 'accounts' ? enhancedAccountTable() : activeTab === 'reviews' ? enhancedReviewPanel() : activeTab === 'settings' ? settingsPanel() : enhancedChatWorkspace();
  icons();
