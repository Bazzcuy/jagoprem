...
function render(options = {}) {
  ...
  const titles = { overview: 'Ringkasan', products: 'Produk', orders: 'Pesanan', accounts: 'Akun Pembeli', reviews: 'Ulasan', chats: 'Chat Pembeli', settings: 'Pengaturan' };
  document.querySelector('#adminTitle').textContent = titles[activeTab] || 'Dashboard';
  content.innerHTML = activeTab === 'overview' ? statsPanel() + orderTable() : activeTab === 'products' ? productTable() : activeTab === 'orders' ? orderTable() : activeTab === 'accounts' ? accountTable() : activeTab === 'reviews' ? reviewPanel() : activeTab === 'settings' ? settingsPanel() : chatWorkspace();
  icons();
  if (activeTab === 'chats') {
...
content.addEventListener('input', (event) => {
  if (!['adminSearch', 'accountSearch', 'reviewSearch'].includes(event.target.id)) return;
  const query = event.target.value.toLowerCase();
  const selector = event.target.id === 'adminSearch' ? '[data-product-row]' : event.target.id === 'accountSearch' ? '[data-account-row]' : '[data-review-row]';
