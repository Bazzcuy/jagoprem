let adminState = { products: [], orders: [], chats: [], users: [], reviews: [], settings: { maintenance: false, maintenanceMessage: '' } };
...
function render(options = {}) {
  ...
  const titles = { overview: 'Ringkasan', products: 'Produk', orders: 'Pesanan', accounts: 'Akun Pembeli', chats: 'Chat Pembeli', settings: 'Pengaturan' }; document.querySelector('#adminTitle').textContent = titles[activeTab];
  content.innerHTML = activeTab === 'overview' ? stats() + orderTable() : activeTab === 'products' ? productTable() : activeTab === 'orders' ? orderTable() : activeTab === 'accounts' ? accountTable() : activeTab === 'settings' ? settingsPanel() : chatWorkspace();
  icons();
...
content.addEventListener('submit', async (event) => {
  if (event.target.id === 'maintenanceForm') { event.preventDefault(); const button = event.target.querySelector('button[type="submit"]'); button.disabled = true; try { const form = new FormData(event.target); const updated = await api('/api/admin/settings', { method: 'PUT', body: JSON.stringify({ maintenance: form.get('maintenance') === 'on', maintenanceMessage: form.get('maintenanceMessage') }) }); adminState.settings = updated; render(); toast(updated.maintenance ? 'Maintenance mode diaktifkan' : 'Toko kembali aktif'); } catch (error) { button.disabled = false; toast(error.message); } return; }
