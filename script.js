const STORE_CONFIG = {
  ...
  staticQris:
    "00020101021126570011ID.DANA.WWW011893600915303270621302090327062130303UMI51440014ID.CO.QRIS.WWW0215ID10265345984810303UMI5204481453033605802ID5914Webtokoku plus6014Kota Palembang61053026663040D38",
};
...
const OFFICIAL_PRIVATE_DESCRIPTION =
  "Akun resmi dan bukan akun ilegal. Akses bersifat privat, bukan sharing, dengan garansi 30 hari sesuai ketentuan penggunaan JagoPrem.";
...
const DOLA_PRODUCT = {
  ...
  access: "Dola AI",
  description:
    "Dola AI adalah asisten chat AI untuk percakapan, menulis, menerjemahkan, coding, mencari inspirasi, dan membahas berbagai topik. Produk aktif 1 bulan sesuai ketentuan penggunaan JagoPrem.",
};
...
const state = {
  ...
  reviewFilter: "all",
  deviceId:
    localStorage.getItem("jagoprem_device_id") ||
    (crypto.randomUUID
      ? crypto.randomUUID()
...
function persistCart() {
  localStorage.setItem("jagoprem_cart", JSON.stringify(state.cart));
  updateCart();
}
...
function productProfile(item) {
  ...
  const description =
    item.description ||
    match?.[1] ||
    `Akses premium ${item.title} dengan detail penggunaan yang dikirimkan langsung oleh admin JagoPrem.`;
  ...
  return {
    ...
    access:
      cat === "AI & produktivitas"
        ...
        : item.access ||
          (isDevApi
            ? "API Key JagoPrem LLM (api.jagoprem.shop)"
...
function reportModal() {
  ...
  const orderOptions = orders
    .map(
      (order) =>
        `<option value="${escapeHtml(order.id)}">${escapeHtml(order.id)} · ${escapeHtml(order.items?.[0]?.title || "Pesanan JagoPrem")}</option>`,
    )
    .join("");
  openModal(
...
function checkoutModal() {
  ...
  if (voucherInput) {
    voucherInput.placeholder = "Contoh: JAGOBARU10";
    ...
    ?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          document.querySelector("#checkoutTotalRow")?.querySelector("span")
            ?.textContent &&
            (document.querySelector("#checkoutTotalRow b").textContent = rupiah(
              cartTotal() - data.discount,
            ));
        }
      } catch {
        feedback.className = "voucher-feedback error";
...
function customerIdentity() {
  const last = JSON.parse(
    localStorage.getItem("jagoprem_last_order") || "null",
  );
...
ChatGPT Plus berupa akun privat, bukan sharing, dan mendukung Codex.`;
  ...
  if (text.includes("poin") || text.includes("jagopoin"))
    return "JagoPoin didapat setelah pesanan berstatus selesai. Jumlah poin berbeda pada tiap produk dan dapat dilihat di akun. Katalog penukaran sedang disiapkan.";
...
document
  ...
  .forEach((button) =>
    button.addEventListener("click", () =>
      notify(
        "Akun media sosial sedang mengalami gangguan. Hubungi Telegram @jagopremofficial.",
      ),
    ),
  );
...
async function loadStore() {
  try {
    ...
    try {
      cached = JSON.parse(
        localStorage.getItem("jagoprem_catalog_cache") || "[]",
      );
    } catch {
      cached = [];
    }
...
async function loadUserSession() {
  ...
  Object.keys(localStorage)
    .filter((key) => key.startsWith("jagoprem_account_"))
...
document.querySelector("#infoButton").addEventListener("click", () => {
  ...
  openModal(
    `${modalHead("Informasi jagoprem.shop")}<div class="modal-body"><div class="about-points"><div><i data-lucide="building-2"></i><span><b>PT Jago Premium Data</b><small>Layanan akun premium private dengan informasi produk yang transparan.</small></span></div><div><i data-lucide="package-check"></i><span><b>Stok terpantau</b><small>Katalog toko dan dashboard admin menggunakan data yang sama.</small></span></div><div><i data-lucide="qr-code"></i><span><b>QRIS sesuai tagihan</b><small>Nominal pembayaran dibuat sesuai total checkout.</small></span></div><div><i data-lucide="message-circle"></i><span><b>Bantuan langsung</b><small>Produk dikirim dan kendala dibantu melalui WhatsApp.</small></span></div></div><button class="submit-button" type="button" data-open-admin-chat>Hubungi tim operasional</button></div>`,
  );
});
modalLayer.addEventListener("click", (event) => {
  ...
  if (event.target.closest("[data-login-help]"))
    notify(
      "Pemulihan akun sedang mengalami gangguan. Hubungi @jagopremofficial.",
    );
});
// ---- SYNC SYSTEM ----
...
async function syncUser() {
  try {
    ...
    const res = await fetch(
      `/api/sync?chatId=${state.chatId}&userId=${userId}&lastSync=${encodeURIComponent(lastSyncTime)}`,
    );
    if (!res.ok) return;
    const data = await res.json();
...
function showPromoPopup() {
  openModal(
    `${modalHead("Promo JagoPrem")}<div class="modal-body campaign-popup"><div class="campaign-hero"><span>JAGOPREM DEALS</span><h2>Lebih hemat untuk akun premium private.</h2><p>Pilih kode yang paling sesuai dengan pesananmu.</p></div><div class="campaign-vouchers"><button type="button" data-copy-voucher="JAGOBARU10"><span>PELANGGAN BARU</span><strong>JAGOBARU10</strong><small>Diskon 10% • minimal Rp25.000</small><i data-lucide="copy"></i></button><button type="button" data-copy-voucher="HEMAT5000"><span>HEMAT LANGSUNG</span><strong>HEMAT5000</strong><small>Potongan Rp5.000 • minimal Rp50.000</small><i data-lucide="copy"></i></button><button type="button" data-copy-voucher="JAGOPREM7"><span>PAKET PREMIUM</span><strong>JAGOPREM7</strong><small>Diskon 7% • minimal Rp75.000</small><i data-lucide="copy"></i></button></div><div class="campaign-note"><i data-lucide="bot"></i><span>Bayar lewat QRIS dinamis, lalu detail produk dikirim oleh bot WhatsApp.</span></div><button class="confirm-button" type="button" data-close-modal>Mulai belanja</button></div>`,
  );
  ...
  document
    ...
    ?.setAttribute("data-copy-voucher", "JAGOPREM7");
  const weeklyVoucher = document.querySelector(
    '[data-copy-voucher="JAGOPREM7"]',
  );
  if (weeklyVoucher)
    weeklyVoucher.innerHTML =
      '<span>PROMO MINGGUAN</span><strong>JAGOPREM7</strong><small>Diskon 7% · minimal Rp75.000</small><i data-lucide="copy"></i>';
  sessionStorage.setItem("jagoprem_campaign_seen", "1");
  icons();
}
